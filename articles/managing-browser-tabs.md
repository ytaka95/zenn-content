...
最近PCで使っているブラウザのタブを開きすぎてメモリ不足になってしまい、これは本格的にどうにかしないといけないと思い立ちました。どう「タブ開きすぎ問題」と向き合うかについて、私なりの考え方や運用方法をご紹介します。特定のツールに依存せずなるべく汎用的な形でまとめております。
...
大量のタブをどう管理するかということを追求しても結局タブは増えるだけで本質的な解決にはなりません。ブラウザのタブは大量に立ち上げておくようには作られておらず、ある程度の数に抑えておく必要があります。考えるべきはどうすればタブを消せるのかということだったのです。これに気づかせてくれたArcありがとう！（ちなみに今はChromeを使っています）

## 我々はなぜタブを残すのか

必要なものを消すことはできません。今まで開いていたタブは何かの目的を持って開いていたはずです。まずはそれを整理してみましょう。私の場合は以下に分類されました。
