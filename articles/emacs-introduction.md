...
最近ふと会社で「Emacsの紹介をしてほしい」と依頼を受けました。好き嫌いがはっきり出そうなツールなのであまり他人にEmacsを強く推薦するつもりは無いのですが、しかしどんな知識も知らないというのはもったいないことですし知りたいと思ったときに知れないというのはもっともったいないです。

自分でもEmacsって何が良いんだろう、ということは昔からあまりはっきりと言語化してこなかったですし、意外とEmacsのことを語る記事も多くないかなと思い、良い機会なので私の観点でEmacsの良さを書いていこうと思います。Emacsのことを知りたいと思った人の助けになれば幸いです。

## 免責

Vimなどの他のエディタやIDE等と比較するものではありません。ましてや優劣をつけるものではありません。どんなツールも好みのものを使えばいいと思います。

ただもしかしたら表現上Emacsを使うことやEmacsを使う人に対して失礼な部分があるかもしれません。事実から大きく外れたことは書かれていないと思いますが、もし読んでいて気分を害された方がいたらコメントをください。

## Emacsを使い始めるまで

まずはEmacsを使えるようになるまでをまとめておきます。いやいや普通に良いところを書けばいいじゃないかと思われるかもしれません。一理あります。しかしEmacsにはEmacsを便利に使うための推奨設定というものがあります（それがデフォルトで実現されていないというのがまたEmacsらしさがありますね）。そういった事前準備無しにEmacsを触ってみて「使えねえな」となってしまっては大変もったいないです。ちょっとの手間で手に入る最高のツールが目の前にあるのに。
...
インストール方法はいろいろありますので公式サイトをご覧ください。
...
以下を `~/.emacs.d/init.el` に記述します。これはEmacsの設定ファイルで起動時に自動で読み込まれるものです。今後設定類は基本的にこのファイルに記述していきます。
...
GUI版だとウインドウサイズとかフォントとかをいじれますし、マウスポインタを使うこともできます（使うかどうかはおいておいて）。ターミナルの設定との切り分けや相性に惑わされることも減るかもしれません（TrueColorとかキーバインドとか）。
...
以下ではEmacs特有の（と思われる）特長をご紹介します。

### キーバインド

Emacsの特徴がControlキーを基本とした

- Macでも使える
- モードの変更が不要 → VSCodeなど他のシステムでの利用に都合が良い

### すべてをバッファとして扱える

何を言っているかわからないと思います。私もどう表現していいかわかりませんでした。例を示します。

例えばGitで管理しているプロジェクトにおける開発中に昔の変更で消してしまった箇所の一部分を復元したくなった、という場面を考えます。いろいろ方法はあるかと思いますが、例えば以下のような手順が考えられます。
...
具体的に何が良いのかという話ですが、まず設定がプログラミング言語で書けるというのはとても便利です。キーバインドとかの設定で条件分岐とかループとか関数とか使えるわけですね。毎分自動でキーバインドを変えることもできます。やりませんけど。ちなみに前述した `Control + h` の設定も、よく見ると関数に２つの引数を渡しているという形ですね。
...
またLispで表現できればいいので、例えばメーラーやブラウザも動きます。[音楽プレイヤー](https://www.emacswiki.org/emacs/%E3%83%9F%E3%83%A5%E3%83%BC%E3%82%B8%E3%83%83%E3%82%AF%E3%83%97%E3%83%AC%E3%82%A4%E3%83%A4%E3%83%BC)とかもあります。狂ってますね。ちなみにEmacsでメールをするのはわりとメジャーです。

あとはスクリプト言語なので例えば誰かが作ったライブラリを手元で修正して動かしてみたりということも簡単です。[GitHub上のinit.elファイル](https://github.com/search?q=path%3A%2F%28%5E%7C%5C%2F%29init%5C.el%24%2F&type=code)を見て参考にするのも良いですね。起動後にインタラクティブに設定を上書き（式を評価）することも簡単です。選択範囲だけ評価とかもできます。

Emacsにハマるとカスタマイズが楽しすぎてコーディングよりもカスタマイズの時間の方が長くなること請け合いです。最高です。
...
前述したGitクライアントです。過去のコミット検索とかブランチ移動とか行ごとのstageとかいろいろEmacs上でできるのは大変便利です。

### neotree

あまり細かい紹介はしませんが、ディレクトリをツリー状に表示します。VSCodeなんかにもついている機能ですね。これももちろんEmacsの検索コマンドがつかえるので普段の流れでファイル検索（というかもはやカーソル移動）が可能です。

### consult

ちょっと概念にクセがありますが、Emacs内蔵の対話システムを置き換えてより便利にするものです。

### eglot (LSP)

Language Server ももちろんあります。これはあまり特筆することでもないですが。
